...
class ClientesModel extends Mysql{
	...
	public function selectCliente(int $idpersona){
		...
		$sql = "SELECT idpersona,identificacion,nombre,apellido,telefono,email_user,nit,nombrefiscal,direccionfiscal,status, DATE_FORMAT(datecreated, '%d-%m-%Y') as fechaRegistro 
						FROM persona
						WHERE idpersona = $this->intIdUsuario and rolid = 15";
	...
	public function updateCliente(int $idUsuario, string $identificacion, string $nombre, string $apellido, int $telefono, string $email, string $password, string $nit, string $nomFiscal, string $dirFiscal){
		...
		if(empty($request)){
			if($this->strPassword  != ""){
				$sql = "UPDATE persona SET identificacion=?, nombre=?, apellido=?, telefono=?, email_user=?, password=?, nit=?, nombrefiscal=?, direccionfiscal=? 
				...
				$sql = "UPDATE persona SET identificacion=?, nombre=?, apellido=?, telefono=?, email_user=?, nit=?, nombrefiscal=?, direccionfiscal=? 
								WHERE idpersona = $this->intIdUsuario ";
				$arrData = array($this->strIdentificacion,
													$this->strNombre,
													$this->strApellido,
